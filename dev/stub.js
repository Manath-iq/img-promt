...
(() => {
  const ANALYSIS = {
    ...
    prompt: 'A woman in her early thirties wearing a sand-coloured linen suit stands three-quarters to camera in an empty studio, hands in pockets, looking straight into the lens. Warm grey seamless paper fills the background with almost no depth, a soft shadow pooling at her feet. A single large softbox sits high and left at 45 degrees, wrapping her face in soft contour light while a white reflector opens the shadows on the right. Shot on 85mm at f/2.2, waist-up framing, subject placed right of centre with a third of the frame left empty. Muted beige palette, matte skin, crisp linen texture, calm and composed mood.',
    prompt_style_only: '{{SUBJECT}} photographed in an empty studio against warm grey seamless paper, a single high-left softbox at 45 degrees with a white reflector opening the shadows. Shot on 85mm at f/2.2, waist-up framing, subject right of centre with a third of the frame empty. Muted sand-and-graphite palette, matte surfaces, soft contour light, calm editorial mood.',
  };
  ...
  async function respond(msg) {
    ...
    switch (msg.type) {
      ...
      case 'REBUILD': {
        ...
        e.analysis = analysis;
        e.prompts = { [msg.target]: { replicate: `[${msg.target}] ` + analysis.prompt, styleOnly: analysis.prompt_style_only } };
